...
class LeetCoinAPIClient:
    ...
    def getIncrementBTC(self):
        return self.incrementBTC

    def getRakePercentage(self):
        return self.leetcoinRakePercentage + self.serverRakeBTCPercentage

    def isPromo(self):
        return self.no_death_penalty
    ...
    def recordKill(self, victim_64, attacker_64):
        ...
        a_index, attacker = self.getPlayerObjByPlatformID(victim_64)
        kick_player = False
    ...
    def getConvertedAmountValue(self, steam_64, amount_in_satoshi):
        """ Convert inputted amount for player and give it in their default currency """
        index, player_obj = self.getPlayerObjByPlatformID(steam_64)
        if self.debug:
            print ("[1337] getConvertedAmountValue for platformID %s" %steam_64)
            print ("[1337] getConvertedAmountValue for index %s" %index)
            
        if player_obj:
            print("[1337] getConvertedAmountValue - returning %i" %amount_in_satoshi)
            converted_amount = str(round(float(amount_in_satoshi * player_obj.defaultCurrencyConversion),2))
            return "" + converted_amount + " " + player_obj.defaultCurrencyDisplay + " (" + str(round(float(amount_in_satoshi/100),2)) + " Bits)"
        else:
            return "Unable to return amount."
    ...
    def submit_match_results_blank(self):
        player_dict_list = []
        sharedDataSemaphore.acquire()
        authorizedPlayers = self.shareddata.get_authorized_player_list()
        ...
        print("Sending blank match results for reset")
...
#                    self.authorizedPlayerObjectList.append(player_obj) ## TODO GLOBAL VAR!!!
    ...
    def deactivate_player(self, action_args):
        ...
        if index:
            sharedDataSemaphore.acquire()
            #self.shareddata.delete_from_authorized_player_list(index)
            sharedDataSemaphore.release()
